//! A CASCA — falar com o binário `schematize-database`, e nada de domínio.
//!
...
//!
//! ## Por que a janela NÃO depende do crate `database`
//!
...
mod tests {
    ...
    fn o_nome_do_binario_conhece_a_plataforma_e_o_dono() {
        let n = nome();
        assert!(n.starts_with("schematize-database"), "{n}");
        if cfg!(windows) {
    ...
    fn o_caminho_nunca_e_vazio() {
        ...
        assert!(b.to_string_lossy().contains("schematize-database"));
    }
    ...
    fn o_comando_usa_caminho_absoluto_e_o_eco_bate() {
        let g = "/home/u/.cargo/bin/schematize-database";
        ...
        let ruim = comando_para_terminal("schematize-database", &["limits"]);
