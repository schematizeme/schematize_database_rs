...
//!
//! **Onde:** `schematize-database desktop --install` / `--remover`, e o `schematize-market`
...
//!
//! **Com a janela instalada** (`schematize-database-gui`, que sai do mesmo repo pelo
//! ADR-0020): `Exec=` aponta para ela e `Terminal=false`.
//!
//! **Sem a janela:** cai em `introspect --help` com `Terminal=true`. Um `.desktop` com
//! `Terminal=false` apontando para uma CLI abriria um processo sem janela que termina no mesmo
...
pub const ID: &str = "schematize-database";

/// Nome do executável da janela — o segundo `[[bin]]` deste repo (ADR-0020).
pub const GUI_BIN: &str = "schematize-database-gui";
...
pub fn resolver_gui(bin_proprio: &Path) -> Option<PathBuf> {
    let nomes: &[&str] =
        if cfg!(windows) { &["schematize-database-gui.exe", GUI_BIN] } else { &[GUI_BIN] };
...
pub fn render(bin: &Path, icone: &Path, gui: Option<&Path>) -> String {
    ...
    // A CLI recebe `introspect --help`, que IMPRIME algo útil — um `--help` do comando que a
    // pessoa vai querer. O binário puro imprimiria o help do clap e sairia.
    let (exec, terminal) = match gui {
        Some(g) => (format!("{}", g.display()), "false"),
        None => (format!("{} introspect --help", bin.display()), "true"),
    };
    format!(
        "[Desktop Entry]\n\
         Type=Application\n\
         Name=schematize Database\n\
         GenericName=Schema, SQL e migration\n\
         Comment=Leia o schema de um banco, modele e gere SQL ou migration\n\
         Exec={exec}\n\
         Icon={}\n\
         Terminal={terminal}\n\
         Categories=Development;Database;\n\
         Keywords=banco;database;schema;sql;migration;sqlite;postgres;\n\
         StartupNotify=false\n\
...
///
/// **Onde:** `schematize-database desktop --install`.
///
...
///
/// **Onde:** `schematize-database desktop --remover`. Os ícones ficam: são inertes, e apagá-los
...
mod tests {
    ...
    /// mínimo, então `Exec=schematize-database` não acharia o binário em `~/.cargo/bin`.
    #[test]
    fn o_exec_e_absoluto_com_e_sem_janela() {
        let bin = Path::new("/home/u/.cargo/bin/schematize-database");
        let gui = Path::new("/home/u/.cargo/bin/schematize-database-gui");

        let sem = exec_de(&render(bin, Path::new("/i/x.png"), None));
        assert!(sem.starts_with("/home/u/.cargo/bin/schematize-database "), "{sem}");

        let com = exec_de(&render(bin, Path::new("/i/x.png"), Some(gui)));
        assert_eq!(com, "/home/u/.cargo/bin/schematize-database-gui");
        assert!(!com.starts_with("schematize-database-gui"), "relativo depende do PATH do DE");
    }
    ...
    fn com_a_janela_instalada_o_icone_abre_a_janela() {
        let t = render(
            Path::new("/b/schematize-database"),
            Path::new("/i/x.png"),
            Some(Path::new("/b/schematize-database-gui")),
        );
        assert!(t.contains("Terminal=false"), "janela com Terminal=true abre terminal atrás");
        let exec = exec_de(&t);
        assert!(exec.ends_with("schematize-database-gui"), "{exec}");
        assert!(!exec.contains("introspect"), "a janela não recebe subcomando: {exec}");
    }
    ...
    fn sem_a_janela_o_icone_ainda_faz_alguma_coisa() {
        let t = render(Path::new("/b/schematize-database"), Path::new("/i/x.png"), None);
        assert!(t.contains("Terminal=true"), "CLI com Terminal=false é um clique que não faz nada");
        assert!(exec_de(&t).contains("introspect --help"), "o clique tem de IMPRIMIR algo");
    }
    ...
    fn o_icone_do_desktop_e_o_png_que_o_app_grava() {
        ...
        let t = render(Path::new("/b/x"), Path::new("/i/schematize-database.png"), None);
    ...
    fn a_classe_de_janela_e_a_da_janela() {
        ...
        assert!(!t.contains("StartupWMClass=schematize-database\n"), "é a classe do CLI: {t}");
    }
    ...
    fn instala_e_remove_no_disco() {
        ...
        let bin = home.join(".cargo/bin/schematize-database");
        let p = instalar_com_gui(&home, &bin, None).expect("instala");
        assert!(p.is_file(), "{}", p.display());
        assert!(
            home.join(".local/share/icons/hicolor/256x256/apps/schematize-database.png").is_file(),
